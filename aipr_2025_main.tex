...
\begin{document}
%
\title{Wavelets and Colon Cancer: An Inside Look}
%
...
\begin{figure}[h!]
\centering
\includegraphics[width=\textwidth]{Images/Tumor_GRF_41.png}
\caption{Top row: A grayscale tumor tissue patch, its power spectrum, and scattering coefficients. Bottom row: Realization of a Gaussian Random Field (GRF), its power spectrum, and scattering coefficients. Power spectra are identical but scattering coefficients differ.}
\label{fig:spectrum}
\end{figure}
...
\begin{figure}[h!]
    \begin{center}
    \includegraphics[width=1.0\textwidth]{Images/scat_tree.pdf}
    \caption{Two-layer scattering tree. Orange nodes: scattering coefficients; white nodes: intermediate wavelet coefficients.}
    \label{fig:network}
    \end{center}
\end{figure}
...
\includegraphics[width=\textwidth]{Images/wavelets_J2_angles6.png}
\label{fig:morlet_fb}
\end{figure}
...
However, scattering networks employ predefined Morlet wavelets rather than learned filters, eliminating the need for  training in the feature extraction process while providing mathematical guarantees of translation invariance and Lipschitz continuity to deformations~\cite{mallat2012group,bruna2013invariant}.  For our colorectal tissue classification task, second-order scattering coefficients ($m=2$) capture scale interactions and 4th-order moments that effectively discriminate the complex textural patterns characteristic of different tissue types. As previously noted, this approach is computationally very efficient as extracting features for the entire test set took just 2.42 minutes, while training the multi-class SVM was completed in only 23.82 seconds.

A few limitations should be acknowledged. The wavelets are predefined rather than learned from data, potentially limiting adaptability to dataset-specific characteristics. Validation on additional multi-center datasets is needed to assess generalization beyond the single dataset used here. Finally, the patch-based analysis does not consider whole-slide spatial context or relationships between neighboring tissue regions.
